...
namespace XRTK.WebXR.Providers.Controllers
{
    ...
    public class WebXRController : BaseController
    {
        ...
        public override void UpdateController()
        {
            ...
            var session = WebXRAPI.GetSession();

            if (!session) return;

            var controller = this.ControllerHandedness == Handedness.Left ? session.LeftInput : session.RightInput;
            ...
            IsPositionAvailable = IsRotationAvailable = session.InSession && controller.Available && controller.IsPositionTracked;
